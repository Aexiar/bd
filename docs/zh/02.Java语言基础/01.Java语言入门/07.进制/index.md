...
| 十进制数 | 原码（16 位二进制数）  | 反码（16 位二进制数）  | 补码（16 位二进制数）  |
| -------- | --------------------- | --------------------- | --------------------- |
...
> [!NOTE]
>
> * ① 如果 1 个字节的最小值，在计算机底层是 `1000 0000` ，那么转换为十进制整数，就是 `- 0` ，而在数学中，没有 `- 0` 的概念，或者可以认为 `- 0` 就是 `0` 。
